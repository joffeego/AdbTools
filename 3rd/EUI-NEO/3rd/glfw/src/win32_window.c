...
static DWORD getWindowStyle(const _GLFWwindow* window)
{
    ...
    else
    {
        ...
        {
            // Frameless: keep WS_CAPTION + WS_THICKFRAME so native resize,
            // Aero Snap, and the maximize/minimize animation all work. The
            // caption/frame are hidden by WM_NCCALCSIZE and the resize border
            // is re-added by WM_NCHITTEST.
            style |= WS_CAPTION;
...
static LRESULT CALLBACK windowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (uMsg)
    {
        ...
        case WM_NCCALCSIZE:
        {
            ...
            // Remove the caption and frame from the client area (the client
            // becomes the whole window). When maximized, the window rect is
            // larger than the work area by the border, so inset the client
            // area to keep the content from being clipped off-screen.
            if (wParam == TRUE && IsZoomed(window->win32.handle))
            {
        ...
        case WM_NCHITTEST:
        {
            ...
            // Re-add the resize border that WM_NCCALCSIZE removed.
            const int border = GetSystemMetrics(SM_CXSIZEFRAME) +
                               GetSystemMetrics(SM_CXPADDEDBORDER);
            POINT cursor;
