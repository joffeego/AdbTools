...
void shutdown() {
    ...
    // Don't block shutdown on in-flight worker tasks (a sleeping device poller,
    // a long adb pull/push, or a network download would otherwise freeze the
    // window for seconds). Detach so the process can exit immediately: workers
    // between tasks observe gStopping and exit on their own, and any worker
    // still blocked in a syscall is reclaimed by the OS at process exit.
