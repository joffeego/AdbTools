...
// Global font-size multiplier applied to every text element's fontSize and
// lineHeight at build time (so the settings dialog can scale text only).
inline float& runtimeFontScale() {
...
public:
    ...
    TextBuilder& fontSize(float value) {
        element_->fontSize = std::max(1.0f, value * core::runtimeFontScale());
        return *this;
    }
    ...
    TextBuilder& lineHeight(float value) {
        element_->lineHeight = std::max(0.0f, value * core::runtimeFontScale());
        return *this;
    }
