...
inline float& runtimeUiScaleTarget() {
    static float value = 0.0f;
    return value;
}

inline float& runtimeUiScaleFrom() {
    static float value = 0.0f;
    return value;
}

inline double& runtimeUiScaleAnimStart() {
    static double value = 0.0;
    return value;
}

inline bool& runtimeUiScaleAnimating() {
    static bool value = false;
    return value;
}

float uiScale() {
    const float configured = dslAppConfig().uiScaleValue;
    const float target = runtimeUiScaleTarget() > 0.0f
        ? runtimeUiScaleTarget()
        : (configured > 0.0f ? configured : 1.0f);

    if (!runtimeUiScaleAnimating()) {
        return target;
    }

    const double now = core::window::timeSeconds();
    const double elapsed = now - runtimeUiScaleAnimStart();
    ...
    const double eased = 1.0 - (1.0 - t) * (1.0 - t) * (1.0 - t);  // ease-out cubic
    return runtimeUiScaleFrom() + (target - runtimeUiScaleFrom()) * static_cast<float>(eased);
}
...
void requestFocus(const std::string& id) {
    ...
    detail::dslRuntime().setFocusedId(id);
    detail::requestFullPaint();
}

namespace detail {
inline std::function<void(const std::vector<std::string>&)>& dropHandler() {
    static std::function<void(const std::vector<std::string>&)> handler;
    return handler;
}
} // namespace detail

void setDropHandler(std::function<void(const std::vector<std::string>&)> handler) {
    detail::dropHandler() = std::move(handler);
}

void dispatchDroppedFiles(const std::vector<std::string>& files) {
    if (detail::dropHandler()) {
        detail::dropHandler()(files);
        detail::requestFullPaint();
    }
}
...
bool isAnimating() {
    return detail::dslRuntime().isAnimating() || runtimeUiScaleAnimating();
}
