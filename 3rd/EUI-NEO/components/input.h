...
public:
    ...
    void build() {
        ...
        const float fontScale = core::runtimeFontScale();
        ...
        const float layoutLineHeight = std::max(1.0f, fontSize * fontScale) * 1.2f;
        const float textY = multiline_ ? inset : std::max(0.0f, (height_ - layoutLineHeight) * 0.5f);
        const float textHeight = multiline_ ? std::max(0.0f, height_ - inset * 2.0f) : layoutLineHeight;
        ...
        const InputLayout layout = InputLayout::build(state, textWidth, textHeight, width_, inset, textY, layoutLineHeight, fontFamily_, fontSize, multiline_);
                    ...
                    .content([&] {
                        ...
                        if (focused) {
                            ui_.rect(id_ + ".cursor")
                                ...
                                .size(1.5f, std::max(1.0f, fontSize * fontScale) * 1.18f)
                                .color(style_.cursor)
                                .radius(1.0f)
                                .build();
                        }
...
private:
    ...
    std::string fontFamily_ = "";  // empty = follow the app's default/global font
    bool hasX_ = false;
    bool hasY_ = false;
};
