...
public:
    ...
    void build() {
        ...
        float labelWidth = w;
        if (hasIcon && hasText) {
            // Measure the label so the icon+text group is truly centered instead
            // of the text being left-aligned inside an over-wide padded box.
            const float available = std::max(0.0f, w - iconWidth - gap - metrics_.spacing.section * 2.0f * scale_);
            const float measured = core::TextPrimitive::measureTextWidth(
                text_, {}, font * core::runtimeFontScale(), 400);
            labelWidth = std::clamp(measured, 0.0f, available);
        }
