...
> 🚀 **0.10.10：修「有的电脑打开特别慢」和一个「点了没反应」。**
> - **启动慢的元凶找到了并修掉**：程序启动时会扫描 PATH 找 adb/scrcpy，而 **PATH 里如果有连不上的网络路径**（比如公司共享盘、NAS，笔记本不在公司/VPN 断开时），探测每一条会卡在 SMB 连接超时上 —— 实测**2.0 秒 → 21.3 秒**。现在会跳过 UNC 网络路径和不存在的盘符，并给整个扫描加了 1.5 秒上限；同样的 PATH 实测回到 **1.0 秒**。
...
> - 关于杀软：**0.10.1 起主包/安装包确实已经不含 adb/scrcpy 等第三方二进制**，所以如果还报毒，触发点就是**本程序自己的 exe 没有数字签名**（加上 Inno Setup 打包、联网、调用子进程这些行为特征）。这不是代码能消掉的，**唯一根治办法是代码签名**；短期请把文件提交到微软误报申诉入口，并把杀软报的**检测名+文件名**告诉我。> 🪟 **0.10.9：窗口每次打开都居中，尺寸继续沿用上次。**
